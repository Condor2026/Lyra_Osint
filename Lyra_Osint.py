#!/usr/bin/env python3
# ==========================================================
# LYRA PRO - OSINT COMPLETO
# CON MAIGRET (INSTALACIÓN AUTOMÁTICA) Y SHERLOCK (DETECCIÓN)
# OPCIÓN 8: INSTALAR HERRAMIENTAS
# ==========================================================
...
# ==========================================================
# LISTA DE ~120 SITIOS VERIFICADOS (SIN API)
# ==========================================================
...
def instalar_maigret():
    ...
    print(f"{Colors.YELLOW}⚠️ Instalando Maigret...{Colors.RESET}")
    ...
    print(f"{Colors.CYAN}[3/4] Instalando dependencias...{Colors.RESET}")
    ...
    if bin_path.exists():
        print(f"{Colors.GREEN}✅ Maigret instalado en: {venv_dir}{Colors.RESET}")
        ...
        if result.returncode == 0:
            ...
            return True, str(python_path) + " -m maigret"
        else:
            print(f"{Colors.RED}❌ Error instalando Maigret. Intenta manualmente: pip install maigret{Colors.RESET}")
            return False, None

# ==========================================================
# INSTALADOR AUTOMÁTICO DE SHERLOCK
# ==========================================================
def instalar_sherlock():
    """Instala Sherlock mediante pip."""
    print(f"{Colors.YELLOW}⚠️ Instalando Sherlock...{Colors.RESET}")
    try:
        subprocess.run([sys.executable, "-m", "pip", "install", "sherlock-project"], check=True)
        print(f"{Colors.GREEN}✅ Sherlock instalado correctamente.{Colors.RESET}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"{Colors.RED}❌ Error instalando Sherlock: {e}{Colors.RESET}")
        return False
...
class UsernameTracker:
    ...
    def track_maigret(self, username: str) -> Dict:
        if not self.maigret_available:
            return {'error': 'Maigret no está disponible. Usa la opción 8 para instalarlo.'}
    ...
    def track_sherlock(self, username: str) -> Dict:
        if not self.sherlock_available:
            return {'error': 'Sherlock no está instalado. Usa la opción 8 para instalarlo.'}
        ...
        try:
            ...
            results['error'] = 'Sherlock no encontrado.'
        except Exception as e:
...
{Colors.GRAY}            ⚖️ Uso exclusivamente legal         {Colors.RESET}
...
""")
    ...
    def menu(self):
        ...
        print(f"{Colors.ORANGE}[8]{Colors.RESET} 🔧 Instalar Herramientas (Maigret/Sherlock)")
        print(f"{Colors.ORANGE}[0]{Colors.RESET} 🚪 Salir")
    
    def install_tools_menu(self):
        self.clear()
        header("🔧 INSTALAR HERRAMIENTAS")
        print(f"{Colors.WHITE}Selecciona qué herramienta instalar:{Colors.RESET}\n")
        print(f"{Colors.ORANGE}[1]{Colors.RESET} Instalar Maigret (3000+ sitios)")
        print(f"{Colors.ORANGE}[2]{Colors.RESET} Instalar Sherlock (479 sitios)")
        print(f"{Colors.ORANGE}[3]{Colors.RESET} Instalar ambas")
        print(f"{Colors.ORANGE}[0]{Colors.RESET} Volver")
        
        opcion = input(f"\n{Colors.ORANGE}Elige: {Colors.RESET}").strip()
        if opcion == "1":
            success, _ = instalar_maigret()
            if success:
                self.lyra.username.maigret_available = True
                self.lyra.username.maigret_cmd = ['maigret']
            input(f"\n{Colors.GRAY}Presiona Enter...{Colors.RESET}")
        elif opcion == "2":
            success = instalar_sherlock()
            if success:
                self.lyra.username.sherlock_available = True
            input(f"\n{Colors.GRAY}Presiona Enter...{Colors.RESET}")
        elif opcion == "3":
            success1, _ = instalar_maigret()
            if success1:
                self.lyra.username.maigret_available = True
                self.lyra.username.maigret_cmd = ['maigret']
            success2 = instalar_sherlock()
            if success2:
                self.lyra.username.sherlock_available = True
            input(f"\n{Colors.GRAY}Presiona Enter...{Colors.RESET}")
        elif opcion == "0":
    ...
    def username_menu_maigret(self):
        ...
        if not self.lyra.username.maigret_available:
            print(f"{Colors.YELLOW}⚠️ Maigret no está disponible. Instálalo con la opción 8.{Colors.RESET}")
            input(f"\n{Colors.GRAY}Presiona Enter...{Colors.RESET}")
            return
        username = input(f"{Colors.WHITE}Username: {Colors.RESET}").strip()
        if not username:
    ...
    def username_menu_sherlock(self):
        ...
        if not self.lyra.username.sherlock_available:
            print(f"{Colors.YELLOW}⚠️ Sherlock no está disponible. Instálalo con la opción 8.{Colors.RESET}")
            input(f"\n{Colors.GRAY}Presiona Enter...{Colors.RESET}")
            return
        username = input(f"{Colors.WHITE}Username: {Colors.RESET}").strip()
        if not username:
